import type { Account, Currency, FixedExpensePayment, MonthlyIncome, Transaction, Transfer } from '@/lib/types'
...
export function computeFlowForMonth(
  ...
  const linkedExpenseTxIds = new Set(
    monthPayments.map((payment) => payment.transaction_id).filter(Boolean) as string[],
  )
...
function cashFromCobros(
  ...
  return cobroMonths.reduce((sum, monthKey) => {
    const flow = computeFlowForMonth(monthKey, monthlyIncomes, payments, transactions, [], new Set())
...
function cashFromTransactions(
  ...
  const paymentTxIds = new Set(
    payments.map((payment) => payment.transaction_id).filter(Boolean) as string[],
  )
...
export function computeWalletTotals(
  ...
  const spendable = (currency: Currency) => {
    const operating = hasCobros
      ? cashFromCobros(currency, monthlyIncomes, payments, transactions)
      : cashFromTransactions(currency, transactions, payments)
